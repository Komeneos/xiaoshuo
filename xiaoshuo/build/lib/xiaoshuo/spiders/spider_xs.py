...
class SpiderXsSpider(scrapy.Spider):
    ...
    def story_directory_parse(self, response):
        ...
        yield scrapy.Request(url = 'http://www.biquge.tv{}'.format(start), callback=self.full_text_parse,dont_filter=True)
    def full_text_parse(self, response):
        ...
        while 1:
            try:
                yield scrapy.Request(url='http://www.biquge.tv{}'.format(next), callback=self.full_text_parse,dont_filter=True)
                break
            except:
                pass